...
def evaluate_mAP(val_loader, model, configs, logger):
    ...
    with torch.no_grad():
        ...
        for batch_idx, batch_data in enumerate(tqdm(val_loader)):
            ...
            imgs_np = imgs.squeeze(0).permute(1, 2, 0).cpu().numpy()
            #plt.imsave(str(batch_idx)+'.jpg', imgs_np)
            ...
            img_pil = Image.fromarray(imgs_np.astype('uint8'))

            # Save the image as a JPEG file
            #output_path = 'output_image.jpg'
            #img_pil.save(str(batch_idx)+".jpg")
